...
public class fja {
	...
	protected boolean un;
			...
			public void dodajOper(String s, boolean b) throws Greska {
				...
				if(glava==null) 
					if (b==true) throw new Greska();//i ako je binarni, greska je u unosu
						...
						if (s=="π") 
							glava=rep=new Elem(s, fja.PI); //ako je pi, dodajem pi
						else if (s=="(" || s==")")
							glava=rep=new Elem(s, fja.ZAGR); //ako je zagrada, dodajem zagradu 
						else {
								un=true;
								glava=rep=new Elem(s, fja.UN_OP); //ako je unarni, dodajem unarni
						}
				...
				//Ako lista nije prazna, dodajem element na kraj 
					...
					if (b==true)	 
					{
						rep.sledeci=new Elem(s, fja.BIN_OP);
					}
				else {
					if (s=="π") 
						rep.sledeci=new Elem(s, fja.PI); //ako je pi, dodajem pi
					else if (s=="(" || s==")")
						rep.sledeci=new Elem(s, fja.ZAGR); //ako je zagrada, dodajem zagradu 
					else 
						rep.sledeci=new Elem(s, fja.UN_OP); //ako je unarni, dodajem unarni	
				}
					pret=rep;
					rep=rep.sledeci;
			}
			...
			public void dodajCif(String s) {
				if(glava==null) 
					if (s=="x") 
							glava=rep=new Elem(s, fja.PROM);
					else 
						glava=rep=new Elem(s, fja.BROJ);
				else {
					if (rep.ind==BROJ) rep.sadrzaj+=s;
					else {
						if (s=="x")
							{
								rep.sledeci=new Elem(s, fja.PROM);
							}
						else rep.sledeci=new Elem(s, fja.BROJ);
						pret=rep;
						rep=rep.sledeci;
					}
				}
			...
			public void in2post() throws Greska {
				...
				while(next!=null) {
					if(next.ind==fja.BROJ || next.ind==fja.PI ||
							next.ind==fja.PROM) { 
						postfix.OUTPUT(next);
						rank++;
					}
		...
		public double evalExp(double prom) throws Greska {
			...
			while(!postfix.END()) {
				x=postfix.INPUT();
				if(x.ind==fja.BROJ || x.ind==fja.PI
						|| x.ind==fja.PROM) stek.PUSH(x);
				else if (x.ind==fja.UN_OP)
				{
					oprnd=stek.POP();
					if (oprnd.ind==fja.PI) rez=-Math.PI;
					else if(oprnd.ind==fja.PROM) rez=-prom;
					else rez=-Double.parseDouble(oprnd.sadrzaj); //kao unarni operand se eventualno moze pojaviti minus
					stek.PUSH(new Elem(Double.toString(rez), fja.BROJ));
				}
				else if(x.ind==fja.BIN_OP) {
					double o[]=new double[2];
					for(int i=0; i<2; i++) {
						 Elem oper=stek.POP();
						 if (oper.ind==fja.BROJ)
							 o[i]=Double.parseDouble(oper.sadrzaj);
						 else if(oper.ind==fja.PI)
							 o[i]=Math.PI;
						 else o[i]=prom;
					}
		...
		public void Unarni(Elem p,double x) throws Greska{
			...
			while(priv!=null) {
				if (priv.ind!=fja.UN_OP )
				{
					...
					if (sled.ind==fja.BIN_OP) throw new Greska();
					else {
						fja podf=new fja(); //podfunkcija 
		...
		public double aprox(double x) throws Greska {
			...
			if (glava.equals(rep)) {
				if(glava.ind==fja.PI) return Math.PI;
				if (glava.ind==fja.BROJ) return Double.parseDouble(glava.sadrzaj);
				else if (glava.ind==fja.PROM) return x;
