...
//	private boolean unisti=false;
	...
	private class OperAkcija implements ActionListener{
		...
		public void actionPerformed(ActionEvent dog) {
			...
			//enable dugmica za operacije
				if (op=="π" || op==")") {
					for(int i=0; i<19; i++) operacije[i].setEnabled(true);
					operacije[19].setEnabled(brZagrada>0);
				}
				else	
					for(int i=0; i<20; i++) {
						if(i<5) operacije[i].setEnabled(false);
						else if(i==19) 	operacije[i].setEnabled(false);
						else operacije[i].setEnabled(true);	
					}
			
				if(op=="-") 
					if (staro.length()==0 || staro.charAt(staro.length()-1)=='(')
						try {
							f.dodajOper(op, false);//dodaje ga kao unarni minus
						} catch (Greska e1) {
						...
						try {
							f.dodajOper(op, true);//dodaje ga kao binarni minus
						} catch (Greska e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
	...
	private class BrojAkcija implements ActionListener{
		...
		//	if(cif=="x" && !prethOp) f.dodajOper("*", true); 
